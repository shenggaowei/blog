---
lastUpdated: true
---

## 基础记录

- [面向对象之继承](./js_base/object_extends.md)
- [浏览器网络请求整理](./js_base/browser_request.md)
- [js 数据类型检测](./js_base/data_type.md)
- [正则表达式](./js_base/regexp.md)
- [浏览器元素宽高计算](./js_base/layout_compute.md)
- [我是 div，我要以多种姿势居中](./css/center.md)
- [太长了，我要加省略号..](./css/ellipsis.md)
- [优雅而又聪明的实现两栏布局](./css/two_column_layout.md)

## 基础进阶

- [理解事件循环机制](./js_advance/event_loop)
- [学会函数式编程，远离代码副作用](./js_advance/function_programmer.md)
- [compose 和 curry 函数实现原理](./js_advance/compose_curry.md)
- [动手实现 bind、call、apply 函数](./js_advance/bind_call_apply.md)
- [节流防抖的实现和应用](./js_advance/throttle_debounce.md)
- [浏览器缓存策略--强缓存和协商缓存](./js_advance/browser_cache.md)
- [浏览器缓存策略--PWA](./js_advance/pwa.md)

## 前端工程化

- [自动格式化代码，打造舒适的项目开发环境](./js_engineering/code_formatting)
- [如何发布第一个 npm 包？](./js_engineering/publish_npm_package.md)
- [rollup: 基础配置](./engineering/rollup/use.md)

## TypeScript

- [用 interface 或者 type 定义类型有什么区别？](./ts/interface_type.md)
- [常见飘红解决](./ts/issue.md)

## 微信小程序

- [openId 和 unionId 的概念和区别](./mini_app/openid_unionid.md)
- [taro 开发配置](./mini_app/taro_use/index.md)

## React

- [理解合成事件](./library/react/syntheticEvent.md)
- [理解 react key](./library/react/key.md)
- [setState 为什么是异步的？](https://github.com/facebook/react/issues/11527#issuecomment-360199710)
- [setState 何时会批量执行？](https://stackoverflow.com/questions/48563650/does-react-keep-the-order-for-state-updates/48610973#48610973)
- [如何实现 setInterval hook？](https://overreacted.io/zh-hans/making-setinterval-declarative-with-react-hooks/)
- [react native 动画应用](./library/react/react_native_animated.md)
